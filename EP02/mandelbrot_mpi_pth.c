...
void init(int argc, char *argv[]){
    if(argc < 2){
        printf("usage: mpirun -np number_of_process ./mandelbrot_mpi_pth number_of_threads\n");
        printf("example:\n");
        printf("mpirun -np 4 ./mandelbrot_mpi_pth 2\n");
        exit(0);
    }
    else{
...
void compute_y_axis(void *_data){
    ...
    int process_start_index = data->process_start_index;
    int start_index = data->start_index;
    int end_index = data->end_index;
    uint8_t *result_array = data->result_array;
...
void compute_mandelbrot(int starting_from, int ending_to, uint8_t *result_array){
    ...
    int offset = starting_from;
    
    for(int i = 0; i < number_of_threads; i++) {
        thread_datas[i].start_index = offset;
        thread_datas[i].end_index = offset + bulk_size;
        thread_datas[i].result_array = result_array;
        thread_datas[i].process_start_index = starting_from;
        offset += bulk_size;
    }
    ...
    */
    ...
    if (rank == RANK_ZERO){
        ...
        if(num_process == 1){
        	...
        	uint8_t *result_array;
        	result_array = (uint8_t *)malloc(i_x_max * i_x_max * sizeof(uint8_t));
