...
void init(int argc, char *argv[]){
    if(argc < 2){
        printf("usage: mpirun -np number_of_process ./mandelbrot_mpi_omp number_of_threads\n");
        printf("example:\n");
        printf("mpirun -np 4 ./mandelbrot_mpi_omp 2\n");
        exit(0);
    }
    else{
...
void compute_y_axis(int process_start_index, int starting_index, int ending_index, uint8_t *result_array){
    uint8_t iteration;
    for(int i_y = starting_index; i_y < ending_index; i_y++){
        ...
        for(int i_x = 0; i_x < i_x_max; i_x++){
            ...
            result_array[(i_y - process_start_index)*i_y_max+i_x] = iteration;
...
void compute_mandelbrot(int starting_from, int ending_to, uint8_t *result_array){
    ...
    // int max = ceil(i_y_max / ((float) number_of_threads)); // maximum value for the y coordinates
    int bulk_size = (ending_to - starting_from) / number_of_threads;

    int process_start_index = starting_from;
    
    #pragma omp parallel for num_threads(number_of_threads)
    for(int i = 0; i < number_of_threads; i++){
        int start = process_start_index + (i * bulk_size);
        int end = start + bulk_size;
        compute_y_axis(process_start_index, start, end, result_array);
    };
    
    // non-parallel task, for the mod part, remaining part
    int remaining = (ending_to - starting_from) % number_of_threads;
    int start = process_start_index + ((number_of_threads-1) * bulk_size) + bulk_size;
